...
public class EcpController {
	...
	public String readData(@Valid EcpModel ecpModel, Model uiModel,
		...
		List<User> activeUsers = userService.findActiveUsersByGroups(ecpModel
				.getGroupIds());
	...
	public String generateDocuments(@Valid EcpModel ecpModel,
		...
		if (ecpModel.getEnabledUserIds() == null) {
			bindingResult.reject("error_ecp_users_not_selected");
		}
		if (bindingResult.hasErrors()) {
			return redirect("users");
		}

		List<String> fileNames = templateService.generateEcpDocuments(
				ecpModel.getEnabledUserIds(), ecpModel.getGroupIds(),
				ecpModel.getDate());
